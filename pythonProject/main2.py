...
pygame.display.set_caption('Game L2')
...
def draw_bg():
    width = bg5.get_width()
    # Variant 2: parallax mid-screen with offsets
    if 2 == 1:
        screen.fill(BG)
        for i in range(5):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not inventory.inv_open:
                ...
                if event.key == pygame.K_SPACE and player.in_air:
                    ...
                    gliding = False
                # Album/Text hotkeys
                if event.key == pygame.K_a: run_external_script("Album2.py")
                if event.key == pygame.K_p: run_external_script("text2.py")
