...
class Game:
    def __init__(self):
        ...
        self.background = pygame.image.load("img/bgEnd.png")
        ...
        self.player = Player("perso", self)
        self.camera = Camera(self)
        self.flower = FlowerSeed(self)
        ...
        self.x = 0
        self.y = self.screen.get_height() - self.player.rect.h
    ...
    def check_collision(self):
        ...
        if not self.is_on_ground and not self.is_on_platform:
    ...
    def handle_events(self):
        ...
        if not self.flower.planted and self.player.rect.colliderect(self.flower.rect):
            if pygame.key.get_pressed()[pygame.K_f]:
                # if player.has_watering_can:
                self.flower.planted = True
                self.flower.growing = True




    def update_animation(self):
        if self.player.on_air:
            self.player.update_action(3)  # 2: jump
        elif self.player.is_running:
            self.player.update_action(2)  # 2: run
        elif self.player.is_moving and not self.player.is_running:
